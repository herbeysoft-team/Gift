...
exports.getmynotification = async (req, res) => {
  ...
  try {
    const notifications = await db.getall(
      "SELECT n.*, up.fullname AS content_owner_name, user.fullname AS username FROM notification n LEFT JOIN userprofile up ON (n.content_owner = up.id) OR (n.content_owner = 0 AND n.content_owner_no = up.phone_no) LEFT JOIN userprofile user ON n.userId = user.id WHERE up.id = ? ORDER BY n.date DESC LIMIT 20",
      [userId?.userId]
    );
    if (notifications) {
        if (notifications) {
